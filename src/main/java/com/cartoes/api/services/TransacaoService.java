...
public class TransacaoService {
	...
	public Transacao salvar(Transacao transacao) throws ConsistenciaException {
		...
		if (!cartao.isPresent()) {
			...
			throw new ConsistenciaException("Nenhum cartao de numero: {} foi encontrado",
					transacao.getCartao().getId());
		}
		...
		if (cartao.get().getBloqueado() == true) {
			log.info("Não é possível incluir transações para este cartão, pois o cartao de id {} encontra-se bloqueado",
					transacao.getCartao().getId());
			throw new ConsistenciaException(
					"Não é possível incluir transações para este cartão, pois cartao de id {} encontra-se bloqueado",
					transacao.getCartao().getId());
		}

		if (cartao.get().getDataValidade().before(dataAtual)) {
			log.info("Não é possível incluir transações para este cartão, pois o cartao de id {} encontra-se vencido.",
					transacao.getCartao().getId());
			throw new ConsistenciaException(
					"Não é possível incluir transações para este cartão, pois o cartao de id {} encontra-se vencido",
					transacao.getCartao().getId());
		}
